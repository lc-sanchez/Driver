...
Usando el código provisto y los comandos de carga y descarga de drivers, se cargó el modulo "miModulo.c" brindado por los docentes.

En primer lugar se instalaron module-init-toolsy linux-headers con los siguientes comandos:
```
>>sudo apt-get update
...
Posteriormente, se ejecutó el comando insmod el cual carga el modulo.  
```
>>sudo insmod ./miModulo.ko
```

Para verificar la carga, introducimos el comando lsmod el cual lista los modulos, luego el comando "rmmod", que descarga el modulo. 
Para ver dichos cambios ejecutamos el comando _dmesg_, con el cual vemos el log del kernel.
```
...
Comandos:  _(imagen: Carga del modulo taichi.png)_
```
>>cd Escritorio
>>make clean
>>make
...
_(imagen: Ejecucion Lectura y Escritura modulo taichi.png)_
...
_(Imagen: Descarga modulo taichi.png)_
